...
player.on("trackStart", (queue, track) => {
	(<QueueMetadata>queue.metadata).textChannel.send(
		`🎶 | Started playing: **${track.title}** in **${queue.connection.channel.name}**! (${track.duration})`
	);
});

player.on("trackAdd", (queue, track) => {
	(<QueueMetadata>queue.metadata).textChannel.send(
		`🎶 | Track **${track.title}** queued! (${track.duration})`
	);
});
...
function getPlayingTrack(guildId: Snowflake) {
	return player.getQueue(guildId).nowPlaying();
}
